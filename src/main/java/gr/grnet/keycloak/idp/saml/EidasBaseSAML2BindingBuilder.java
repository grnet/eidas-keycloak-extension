 ...
 * @version $Revision: 1 $
 */
public class EidasBaseSAML2BindingBuilder<T extends EidasBaseSAML2BindingBuilder> {

	protected static final String COUNTRY = "country";

	protected static final Logger logger = Logger.getLogger(EidasBaseSAML2BindingBuilder.class);
	...
	public class BasePostBindingBuilder {
		...
		protected EidasBaseSAML2BindingBuilder builder;

		public BasePostBindingBuilder(EidasBaseSAML2BindingBuilder builder, Document document)
		...
		public String getHtmlResponse(String actionUrl)
	...
	public static class BaseRedirectBindingBuilder {
		...
		public BaseRedirectBindingBuilder(EidasBaseSAML2BindingBuilder builder, Document document)
				throws ProcessingException {
			this.builder = builder;
			this.document = document;
			if (builder.encrypt)
	...
	public String getSAMLNSPrefix(Document samlResponseDocument) {
		Node assertionElement = samlResponseDocument.getDocumentElement()
				.getElementsByTagNameNS(JBossSAMLURIConstants.ASSERTION_NSURI.get(), JBossSAMLConstants.ASSERTION.get())
				.item(0);
		...
		try {
			...
			XMLEncryptionUtil.encryptElement(
					new QName(JBossSAMLURIConstants.ASSERTION_NSURI.get(), JBossSAMLConstants.ASSERTION.get(),
							samlNSPrefix),
					samlDocument, encryptionPublicKey, secretKey, encryptionKeySize, encryptedAssertionElementQName,
					true);
		} catch (Exception e) {
	...
	public void signAssertion(Document samlDocument) throws ProcessingException {
		Element originalAssertionElement = org.keycloak.saml.common.util.DocumentUtil.getChildElement(
	...
	public static String getSAMLResponse(Document responseDoc)
			throws ProcessingException, ConfigurationException, IOException {
		byte[] responseBytes = org.keycloak.saml.common.util.DocumentUtil.getDocumentAsString(responseDoc)
	...
	public String buildHtml(String samlResponse, String actionUrl, boolean asRequest) {
		...
		if (isNotNull(country)) {
			inputTypes.put(COUNTRY, country);
		}
	...
	public String buildHtmlForm(String actionUrl, Map<String, String> inputTypes) {
		...
		builder.append("<HTML>").append("<HEAD>")
				...
				.append("<TITLE>Authentication Redirect</TITLE>").append("</HEAD>")
		...
		for (String key : inputTypes.keySet()) {
			builder.append("<INPUT TYPE=\"HIDDEN\" NAME=\"").append(key).append("\"").append(" VALUE=\"")
					.append(escapeAttribute(inputTypes.get(key))).append("\"/>");
		}

		builder.append("<NOSCRIPT>").append(
				"<P>JavaScript is disabled. We strongly recommend to enable it. Click the button below to continue.</P>")
				.append("<INPUT TYPE=\"SUBMIT\" VALUE=\"CONTINUE\" />").append("</NOSCRIPT>")
